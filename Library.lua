...
local BaseURL = "https://raw.githubusercontent.com/ehdllhnou/idgrdam/refs/heads/main/"
local CustomImageManager = {}
local CustomImageManagerAssets = {
    TransparencyTexture = {
        RobloxId = 139785960036434,
        Path = "Midgard/assets/TransparencyTexture.png",
    ...
    SaturationMap = {
        RobloxId = 4155801252,
        Path = "Midgard/assets/SaturationMap.png",
...
do
    ...
    function CustomImageManager.AddAsset(
        ...
        CustomImageManagerAssets[AssetName] = {
            ...
            Path = string.format("Midgard/custom_assets/%s", AssetName),
            URL = URL,
...
function Library:RemoveShadow(Holder: GuiObject, All: boolean?)
    for _, Child in ipairs(Holder:GetChildren()) do
        if Child:IsA("UIStroke") and (All or Child.ZIndex == 1) then
            Child.Thickness = 0
        end
    end
end
...
function Library:MakeOutline(Frame: GuiObject, Corner: number?, ZIndex: number?)
    warn("Midgard:MakeOutline is deprecated, please use Midgard:AddOutline instead.")
...
do
    ...
    function Funcs:AddSlider(Idx, Info)
        ...
        function Slider:SetValue(Str)
            ...
            if Slider.Increment and Slider.Increment > 0 then
                Num = math.floor((Num - Slider.Min) / Slider.Increment + 0.5) * Slider.Increment + Slider.Min
                Num = math.clamp(Num, Slider.Min, Slider.Max)
            end
        ...
        Bar.InputBegan:Connect(function(Input: InputObject)
            ...
            while IsDragInput(Input) do
                ...
                if Slider.Increment and Slider.Increment > 0 then
                    RawValue = math.floor((RawValue - Slider.Min) / Slider.Increment + 0.5) * Slider.Increment + Slider.Min
                    RawValue = math.clamp(RawValue, Slider.Min, Slider.Max)
                end
                Slider.Value = Round(RawValue, Slider.Rounding)
    ...
    function Funcs:MakeCollapsible(StartCollapsed: boolean?)
        ...
        local function Apply()
            ...
            if DividerLine then
                DividerLine.Visible = not Collapsed
            end

            if Collapsed then
                if Holder.Size.Y.Offset > 34 then
...
function Library:CreateWindow(WindowInfo)
    ...
    function Window:AddTab(...)
        ...
        function Tab:AddCollapsibleTabbox(Info)
            ...
            local tb = Tab:AddTabbox({ Side = Info.Side })
            tb.BoxHolder.Parent = gb.Container
            gb.Container:FindFirstChildOfClass("UIListLayout").Padding = UDim.new(0, 1)
            tb.Holder:GetPropertyChangedSignal("Size"):Connect(function()
                if gb.Container.Visible then
                    gb:Resize()
                end
            end)
            gb:MakeCollapsible(true)
            gb:Resize()
            return tb
        end
